...
The other API correction came from implementing `AmlWorkItemLifecycleObserver`: `WorkItemLifecycleEvent` has no `workItem()` method. It exposes the entity via `event.source()` cast to `WorkItemEntity`. The factory method `of(...)` also NPEs if `workItem` is null — tests that need a null-snapshot event must use `fromWire()` instead.
